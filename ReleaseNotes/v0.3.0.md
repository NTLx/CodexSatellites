...
- `WidgetSnapshotTransport` keeps the two paths explicit: `widgetContainer` today, `appGroup` once a Developer ID build provisions the App Group. `load()` takes the newest snapshot across transports, so a migration can never serve stale data.
