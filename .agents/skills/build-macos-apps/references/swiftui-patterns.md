...
- Choose the relevant reference from `swiftui-patterns/components-index.md`.
...
- For a new app scaffold, also create one project-local `script/build_and_run.sh` and `.codex/environments/environment.toml` so the Codex app Run button works immediately. Use the exact bootstrap contract from `build-run-debug.md` and its `build-run-debug/run-button-bootstrap.md` file rather than inventing a second variant here.
...
`swiftui-patterns/split-inspectors.md`.
...
- `swiftui-patterns/components-index.md`: entry point for scene and component guidance.
- `swiftui-patterns/windowing.md`: choosing between `WindowGroup`, `Window`, `DocumentGroup`, and window-opening patterns.
- `swiftui-patterns/settings.md`: dedicated settings scenes, `SettingsLink`, and preference layouts.
- `swiftui-patterns/commands-menus.md`: command menus, keyboard shortcuts, focused values, and desktop action routing.
- `swiftui-patterns/split-inspectors.md`: sidebars, split views, selection-driven layout, and inspectors.
- `swiftui-patterns/menu-bar-extra.md`: menu bar extra structure and when it fits.
...
Use `swiftui-patterns/components-index.md` as the entry point. Each component reference should include:
